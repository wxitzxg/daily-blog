...
source: "https://mp.weixin.qq.com/s/IsuE58_Hukeabtv6G9BY0w"
author:
  - "[[野码AI]]"
published:
created: 2026-03-24
description: "Claude Code 正在测试一套全新的项目初始化体系。只要设置 CLAUDE_CODE_NEW_INIT=1，/init 命令的行为会发生根本性变化——它不再只是创建 CLAUDE.md，而是会根据项目目录下的配置文件，自动装配 ..."
category: tips
tags: [init, 配置范式, 项目初始化, agents, hooks]
summary: Claude Code 新版 /init 引入声明式配置体系，通过 .claude/ 目录自动装配 Agent、MCP、Hooks 和 Skills，实现项目启动即完整工作环境。
---
