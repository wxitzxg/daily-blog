...
category: workflow
tags: [SDD, superpowers, gstack, 规范驱动开发, skill组合]
summary: 实践规范驱动开发（SDD）的完整工作流：规划阶段用 gstack 的 office-hours、plan-ceo-review、plan-eng-review、plan-design-review；开发阶段用 superpowers 的 brainstorming、write-a-prd、writing-plans、subagent-driven-development；发布阶段用 gstack 的 review、qa、ship。以 Agent 竞技场项目为例展示完整流程，核心价值是大幅减少返工。
