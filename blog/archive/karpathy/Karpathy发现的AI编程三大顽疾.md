...
summary: 基于Karpathy对AI编程问题的观察，andrej-karpathy-skills项目用四个原则约束AI行为：编码前先思考（不猜不装懂）、简单优先（能少写就不多写）、外科手术式修改（只改该改的）、目标驱动执行（给成功标准而非命令）。一个CLAUDE.md文件即可让AI从”热情过度的实习生”变成”手稳不越界的资深工程师”。
---
