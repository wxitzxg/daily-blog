...
category: karpathy
tags: [llm-wiki, obsidian, 知识管理, 开源项目, 自动化]
summary: 开源项目Obsidian Brain OS介绍，一个AI驱动的个人上下文管理系统。包含：Nightly Pipeline（凌晨自动知识整合）、Personal Ops（AI参谋长）、三层知识架构（READING/WORKING/SYSTEM）、Agent团队（主Agent/Writer/Chronicle/Review）。实现每天凌晨自动跑知识整合，早上生成今日驾驶舱，让知识持续累积而非消失在聊天记录里。
---
