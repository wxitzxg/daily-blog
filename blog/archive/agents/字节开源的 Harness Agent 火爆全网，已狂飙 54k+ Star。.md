...
description:
category: agents
tags: [harness-engineering, deerflow, agent-架构, 字节开源]
summary: 深度解析 Harness Engineering 概念：Agent = Model + Harness。Harness 是包裹在模型外围的运行时控制系统，包含标准化工具集成、上下文工程、状态持久化、子代理编排、验证防护、可观测性六大组件。结合字节开源的 DeerFlow 2.0 和 Claude Code 的实践案例，阐述如何让非确定性模型在确定性框架中稳定运行。
